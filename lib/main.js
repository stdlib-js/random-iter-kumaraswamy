...
function iterator( a, b, options ) {
	...
	if ( !isPositiveNumber( a ) ) {
		throw new TypeError( format( '0Pr7P', a ) );
	}
	if ( !isPositiveNumber( b ) ) {
		throw new TypeError( format( '0Pr7T', b ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0Pr2h', options ) );
		}
		opts = copy( options, 1 );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0Pr35', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
