...
function iterator( a, b, options ) {
	...
	if ( !isPositiveNumber( a ) ) {
		throw new TypeError( format( '0pv71', a ) );
	}
	if ( !isPositiveNumber( b ) ) {
		throw new TypeError( format( '0pv72', b ) );
	}
	if ( arguments.length > 2 ) {
		if ( !isObject( options ) ) {
			throw new TypeError( format( '0pv2V', options ) );
		}
		opts = assign( {}, options );
		if ( hasOwnProp( opts, 'iter' ) ) {
			if ( !isNonNegativeInteger( opts.iter ) ) {
				throw new TypeError( format( '0pv2t', 'iter', opts.iter ) );
			}
		} else {
			opts.iter = MAX_VALUE;
		}
